<template>
  ...
  >
    <row>
      ...
      <column size="100%">
        <select :id="`id-${name}-size-type`" v-model="selection.columns[index].mode">
    ...
    <row>
      ...
      <column size="100%" v-if="selection.columns[index].mode == 'Fixed'">
        <select :id="`id-${name}-size`" v-model="selection.columns[index].size">
          <option value="0" key="0" v-html="0"/>
          <option v-for="option in fixedValues" :value="option" :key="option" v-html="option"/>
        </select>
      </column>
    ...
    <row v-if="selection.columns[index].mode != 'Fixed'">
      ...
      <column size="100%">
        <select :id="`id-${name}-subtraction`" v-model="selection.columns[index].subtraction">
          <option value="0" key="0" v-html="0"/>
          <option v-for="option in fixedSubstractionValues" :value="option" :key="option" v-html="option"/>
        </select>
      </column>
    </row>
    ...
    <row>
      ...
      <column size="100%">
        <select :id="`id-${name}-style`" v-model="selection.columns[index].block">
